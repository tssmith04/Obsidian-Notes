...
9/17
$f(X|\theta)$ is a member of exponential family if $$
f(X|\theta)=h(x)\exp\left( \sum_{i=1}^{k} w_{i}(\theta)T_{i}(x)-A(\theta) \right)
$$
The natural parameter family is $$
f(x|\eta)=h(x)\exp\left( \sum_{i=1}^{k}\eta_{i}T_{i}(x)-A(\eta)  \right)
$$
The natural parameter space is $$
\mathcal{H}=\left\{  \eta:\int h(x)\exp(<\eta,T(x)>)dx<\infty  \right\}
$$
We can write $W=\{ w(\theta):\theta\in\Theta \}$ all the $\theta$'s that you are addressing in your original parameterization. But, $\mathcal{H}$ is all allowable parameters. Thus, $W\subseteq \mathcal{H}$.

##### Curved Exponential Family
If $dim(W)<dim(\mathcal{H})$ then we call $W$ the curved exponential family.

Example 1
$X_{1},\dots,X_{n}\overset{\text{iid}}{\sim}N(\mu,\mu^{2})$ for $\mu\in \mathbb{R}\backslash\{ 0 \}$
Question: Is this a curved exponential family?
$f(x|\mu)=\left( \frac{1}{\sqrt{2\pi\mu^2}} \right)^{n} \exp\left(-\frac{\sum_{i=1}^{n}(x_{i}-\mu)^2}{2\sigma^2}\right)=\frac{1}{(2\pi)^{n/2}}\exp\left( -\frac{1}{2\mu^{2}}\sum_{i=1}^{n}x_{i}^{2}+\frac{1}{\mu}\sum_{i=1}^{n}x_{i}-\frac{n}{2}-n\log \mu \right)$
Yes this is a $k=2$ exponential family with $w_{1}(\mu)=-\frac{1}{2\mu^{2}}$, $w_{2}(\mu)=\frac{1}{\mu}$, $T_{1}(x)=\sum_{i=1}^{n}x_{i}^{2}$, $T_{2}(x)=\sum_{i=1}^{n}x_{i}$ and the remaining constant term is $A(\mu)$.
$w_{1},w_{2}$ and $T_{1},T_{2}$ are linearly independent.
$\mathcal{H}=\left\{  \eta: \int \frac{1}{(2\pi)^{n/2}}\exp\left( \eta_{1}\sum x_{i}^{2}+\eta_{2}\sum x_{i} \right)dx_{1}\dots dx_{n}<\infty  \right\}$
Notice $\mathcal{H}=(-\infty,0)\times \mathbb{R}$. As long as coefficient in front of quadratic sum is negative the second parameter (coefficient of linear term) doesn't matter.
$W=\left\{  \left( -\frac{1}{2\mu^{2}},\frac{1}{\mu} \right):\mu\in \mathbb{R}\backslash\{ 0 \}  \right\}=\left\{  \left( -\frac{y^{2}}{2}, y \right):y\in \mathbb{R}\backslash\{ 0 \}  \right\}\subset \mathcal{H}$. You can graph this and see that $W$ is just a sideways parabola while $\mathcal{H}$ is an entire half space $\{ x<0 \}$.
So yes $N(\mu,\mu^{2})$ is a curved exponential family.

Example 2
$X_{1},\dots,X_{n}\overset{\text{iid}}{\sim}N(\mu,\mu)$ with $\mu>0$
Question: is this a curved exponential family?
$f(x|\mu)=\exp\left( -\frac{1}{2\mu}\sum_{i=1}^{n}x_{i}^{2}+\sum_{i=1}^{n}x_{i}-\frac{n\mu}{2}- \frac{n}{2} \log 2\pi\mu \right)=\exp\left( \sum_{i=1}^{n}x_{i} \right)\exp\left( -\frac{1}{2\mu}\sum_{i=1}^{n}x_{i}^{2}-\frac{n}{2}(\mu+\log(2\pi \mu)) \right)$
Notice in this case we have $h(x)=\exp{(\sum_{i=1}^{n}x_{i}})$, $w_{1}(\mu)=-\frac{1}{2\mu}$, $T_{1}(x)=\sum_{i=1}^{n}x_{i}^{2}$ and $A(\mu)$ is extra term. Thus we have a $k=1$ exponential family, not $k=2$.
This gives us $\mathcal{H}=(-\infty,0)$ (since $\eta=-\frac{1}{2\mu}$ so if $\eta<0$ then the integral is finite) and $W=\left\{  -\frac{1}{2\mu}:\mu>0  \right\}=(-\infty,0)=\mathcal{H}$
So $N(\mu,\mu)$ is not curved!

##### Theorems/Properties for Natural Exponential Families
**Theorem**: Let $f(x|\eta)$ be the natural/canonical exponential family generated by $(T,h)$. Then $\mathcal{H}$ is a convex set and $A(\eta)$ is a convex function.
Proof: Let $\eta_{1},\eta_{2}\in \mathcal{H},\lambda\in(0,1)$. I need to show that $\lambda \eta_{1}+(1-\lambda)\eta_{2}\in \mathcal{H}$ and $A(\lambda \eta_{1}+(1-\lambda)\eta_{2})\leq\lambda A(\eta_{1})+(1-\lambda)A(\eta_{2})$
Define $u(x)=\exp(\lambda<\eta_{1},T(x)>)$ and $w(x)=\exp((1-\lambda)<\eta_{2},T(x)>)$
And let $\frac{1}{p}=\lambda, \frac{1}{q}=(1-\lambda)$. We are doing this so we can use Holder's inequality.
Notice $\int u(x)w(x)h(x)dx=\int h(x)\exp(<\lambda \eta_{1}+(1-\lambda)\eta_{2},T(x)>)dx$. So if we can show that this integral is finite then we have shown $\mathcal{H}$ is convex.
By Holder's inequality we have $$
\int u(x)w(x)h(x)dx\leq \left( \int u(x)^{p}h(x)dx \right)^{1/p}\left( \int w(x)^{q}h(x)dx \right)^{1/q}
$$
$$
=\left( \int \exp(\lambda<\eta_{1},T(x)>)^{1/\lambda}h(x)dx \right)^{\lambda}\left( \int \exp(<\eta_{2},T(x)>)h(x)dx \right)^{1-\lambda}
$$ (in second integral we just did the elimination of power of $1-\lambda$ and coefficient $1-\lambda$ as we could do for first integral). Note that both of these integrals are finite since $\eta_{1},\eta_{2}\in \mathcal{H}$, thus the product of these integrals raised to finite $\lambda,1-\lambda$ respectively is finite. Thus, $\lambda \eta_{1}+(1-\lambda)\eta_{2}\in \mathcal{H}$ so $\mathcal{H}$ is convex.

Notice that $A(\eta)=\log \int h(x)\exp(<\eta,T(x))>)dx$. Then by taking a log we also proved convexity of $A$. I.e. $A(\lambda \eta_{1}+(1-\lambda)\eta_{2})=\log\int u(x)w(x)h(x)dx\leq\lambda A(\eta_{1})+(1-\lambda)A(\eta_{2})$ (by taking log of above final term and bringing down exponents).

Now we will be investigating the derivatives of $A(\eta)$
**Theorem**: If the following hold
1. $f(x,\omega)$ integrable in $\omega$ for all $x$
2. $\frac{\partial}{\partial x}f(x,\omega)$ exists a.s. for all $\omega$
3. $\left| \frac{\partial}{\partial x}f(x,\omega)\right|\leq\theta(\omega),\int\theta(\omega)d\omega<\infty$
Then, $\frac{\partial}{\partial x}\int f(x,\omega)d\omega=\int \frac{\partial}{\partial x}f(x,\omega)d\omega$

**Lemma**:
Let $G(\eta)=\exp(A(\eta))=\int h(x)\exp(<\eta,T(x)>)dx$. The function $G(\eta)$ is infinitely differentiable at $\eta\in \mathcal{H^{o}}$ (interior of $\mathcal{H}$). Moreover, the differentiation can be passed inside the integral. $$
\frac{\partial^{k_{1}+\dots+k_{p}}}{\partial^{k_{1}} \eta_{1}\dots \partial^{k_{p}}\eta_{p}}G(\eta)=\int h(x)\frac{\partial^{k_{1}+\dots+k_{p}}}{\partial^{k_{1}} \eta_{1}\dots \partial^{k_{p}}\eta_{p}}\exp(<\eta,T(x)>)dx
$$
$$
=\int \prod_{i=1}^{p} (T_{i}(x))^{k_{i}}h(x)\exp(<\eta,T(x))dx
$$Here we are basically saying we can take the partial with respect to $\eta_{i}$ $k_{i}$ times and this passes inside the integral to get a nice looking product of the $T_{i}(x)$'s based on how many times the $i$th partial is taken.

Ideas for the proof:
Use the fact that
$$\frac{e^{\epsilon y-1}}{\epsilon}\geq y,\forall\epsilon>0,\forall y\in \mathbb{R}$$
Then we have $\left( \prod_{i=1}^{p}T_{i}^{k_{i}} \right)\exp(<\eta,T(x)>)\leq \exp(<\eta+k\epsilon,T(x)>)$ where $k=(k_{1},\dots,k_{p})$.
In principle since $\eta\in \mathcal{H}^{o}$ there is an open ball $B_{\delta}(\eta)\subset \mathcal{H}$, we can pick $\epsilon>0$ small enough such that $\eta+k\epsilon\in B_{\delta}(\eta)$.
